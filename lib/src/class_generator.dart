...
class ClassGenerator {
  ...
  String generate(DartFormatter _dartFmt) {
    if (!element.isEnum || !element.isPrivate) {
      throw InvalidGenerationSourceError('${element.name} must be a private Enum');
    }
    ...
    try {
      final cls = Class((c) => c
        ...
        ..docs.addAll((type_processor.docCommentsOf(element)) as dynamic)
  ...
  Method get _generateWhenMethod {
    ...
    final assertionCondition = _fields.map((f) => '${getCamelCase(f.name)} == null').join(' || ');
    ...
    for (var field in _fields) {
      final hasObjectAnnotation = type_processor.hasAnnotation<ObjectClass>(field);
      final DartObject? usedClass = type_processor.usedClassFromAnnotation(field);
      ...
      final callbackArgType =
          usedClass != null ? '${usedClass.toTypeValue()!.getDisplayString(withNullability: false)}' : '${field.name}';
  ...
  Method get _generateWhenPartialMethod {
    ...
    _bodyBuffer.writeln('}');
    dynamic d = type_processor.patternMatchingMethodDocComment(
  ...
  Iterable<Constructor> get _generateClassConstructors {
    ...
    final fieldConstructors = _fields.map((field) {
      final annotation = TypeChecker.fromRuntime(UseClass).firstAnnotationOfExact(field);
      var redirectConstructorName = '${field.name}${_isNamespaceGeneric ? '<T>' : ''}';
      final reqParams = <Parameter>[];
      if (annotation != null) {
        final DartObject? usedClass = annotation.getField('type');
        redirectConstructorName = type_processor.usedWrapperNameFromAnnotation(field)!;
      ...
      return Constructor((constructor) => constructor
        ..factory = true
        ..name = '${getCamelCase(field.name)}'
        ..docs.addAll(type_processor.docCommentsOf(field) as dynamic)
        ..optionalParameters
            .addAll(type_processor.hasAnnotation<Data>(field) ? _generateClassConstructorFields(field) : [])
  ...
  Iterable<Parameter> _generateClassConstructorFields(FieldElement element) {
    ...
    return fields.map((e) => Parameter((f) {
          ...
          f
            ...
            ..type = refer(type_processor.dataFieldType(e) as dynamic)
            ..named = true
            ..build();
        }));
  }
  ...
  ImplementedClass _generateObjectClass(FieldElement field) {
    ...
    if (isGeneric) {
      throw InvalidGenerationSourceError('Can\'t use @generic on object classes');
    }
    ...
    final _objectClass = Class((c) => c
      ...
      ..abstract = true
      ..docs.addAll(type_processor.docCommentsOf(field) as dynamic)
      ..extend = refer('${element.name.replaceFirst('_', '')}${_isNamespaceGeneric ? '<T>' : ''}')
      ..annotations.add(references.immutable)
      ..types.addAll(_isNamespaceGeneric ? [references.generic_T] : [])
      ..constructors.addAll([
        Constructor((c) => c
        ...
        Constructor((c) => c
          ...
          ..factory = true
          ..docs.addAll(type_processor.docCommentsOf(field) as dynamic)
          ..redirect = refer('_${field.name}Impl${_isNamespaceGeneric ? '<T>' : ''}')
          ..build())
      ])
      ..build());
  ...
  ImplementedClass _generateDataClass(FieldElement field) {
    ...
    final Method toString = Method((m) {
      final String values = _classFields
          .map((f) => '${type_processor.dataFieldName(f)}: \${this.${type_processor.dataFieldName(f)}}')
          .join(', ');
      m
        ..name = 'toString'
        ..lambda = true
    ...
    Method getProps = Method((m) {
      final String values = _classFields.map((f) => '${type_processor.dataFieldName(f)}').join(',');
      m
        ..name = 'props'
        ..lambda = true
        ..returns = references.object_list
    ...
    final Method copyWithImpl = Method((m) {
      final String values = _classFields.map((e) {
        ...
        var value = '$dataFieldName: $dataFieldName == superEnum ? this.$dataFieldName : $dataFieldName';
      ...
      m
        ...
        ..optionalParameters.addAll(_classFields.map((e) => Parameter((f) => f
          ...
          ..named = true
          ..build())))
        ..returns = Reference('_${field.name}Impl${_isNamespaceGeneric ? '<T>' : ''}')
    ...
    if (isGeneric) {
      if (_classFields
          .where((e) => type_processor.dataFieldType(e)!.contains('<T>') || type_processor.dataFieldType(e) == 'T')
          .isEmpty) {
        throw InvalidGenerationSourceError('${field.name} must have atleast one Generic field');
      }
    }

    if (_classFields
        .any((e) => type_processor.dataFieldType(e)!.contains('<T>') || type_processor.dataFieldType(e) == 'T')) {
      if (!isGeneric) {
        throw InvalidGenerationSourceError('${field.name} must be annotated with @generic');
      }
    }
      ...
      ..constructors.addAll([
        ...
        Constructor((c) => c
          ...
          ..optionalParameters.addAll(_classFields.map((e) => Parameter((f) {
                ...
                f
                  ..name = '${type_processor.dataFieldType(e)} ${type_processor.dataFieldName(e)}'
                  ..named = true
                  ..build();
              })))
          ..redirect = refer('_${field.name}Impl${_isNamespaceGeneric ? '<T>' : ''}')
...
class ImplementedClass {
  ...
  String accept(DartEmitter dartEmitter) => '${abstractClass.accept(dartEmitter)}'
