...
ConstantReader annotationOf<T>(Element obj) => ConstantReader(_typeChecker(T).firstAnnotationOfExact(obj));

ConstantReader? fieldOf<T>(Element obj, String fieldName) => annotationOf<T>(obj).read(fieldName);

Iterable<DartObject> listTypeFieldOf<T>(Element obj, String fieldName) => fieldOf<T>(obj, fieldName)?.listValue ?? [];

bool hasAnnotation<T>(Element obj) => _typeChecker(T).hasAnnotationOfExact(obj);

bool isGeneric(Element element) => _typeChecker(Generic).hasAnnotationOfExact(element);
...
List<String?> patternMatchingMethodDocComment(PatternMatchingMethod method, Element element) {
  final comments = <String?>[];
  comments.add({
    PatternMatchingMethod.when: '/// The [when] method is the equivalent to pattern matching.\n'
        '/// Its prototype depends on the ${element.name} [_type]s defined.',
    PatternMatchingMethod.whenOrElse: "/// The [whenOrElse] method is equivalent to [when], but doesn't require\n"
        "/// all callbacks to be specified.\n"
        "///\n"
        "/// On the other hand, it adds an extra orElse required parameter,\n"
        "/// for fallback behavior.",
    PatternMatchingMethod.whenPartial: '/// The [whenPartial] method is equivalent to [whenOrElse],\n'
        '/// but non-exhaustive.',
  }[method]);
